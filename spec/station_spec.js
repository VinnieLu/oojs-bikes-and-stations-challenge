describe("a bike station", function() {
  ...
  beforeEach(function() {
    ...
    blueBike = new Bike ("23524", "blue", 2017)
    redBike = new Bike ("23465", "green", 2018)
    greenBike = new Bike ("34566", "orange", 2019)

    station = new Station("Divvy", "Wood & Division", [newerBike, olderBike], 4)
  });
  ...
  describe("taking more bikes", function() {
    it("equals nothing", function() {
      station.bikes = [];
      expect(station.bikes).toEqual([]);
    });

    it("equals the two", function() {
      expect(station.bikes).toContain(newerBike, olderBike)
    });

    it("takes more bikes", function() {
      station.takeMore([blueBike, redBike])
      expect(station.bikes).toContain(newerBike, olderBike, blueBike, redBike)
    });
    it("Won't take a bike over capacity", function() {
      station.takeMore([blueBike, redBike, greenBike])
      expect(station.bikes).toContain(newerBike, olderBike, blueBike, redBike)
    })
  })
});
